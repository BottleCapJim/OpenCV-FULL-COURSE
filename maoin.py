...
while True:
    ...
    for (x_eye, y_eye, w_eye, h_eye) in eyes:
        ...
        roi_scale = 0.2  # Specify the scaling factor for ROI size
        ...
        for contour in contours:
            ...
            if area_condition and circularity_condition:
                ...
                radius = int((w + h) / 2.5)  # Adjust the radius to make the iris region bigger
                cv2.circle(frame, (int(x_eye + x + x_offset + w / 2), int(y_eye + y + y_offset + h / 2)),
                           radius, (0, 180, 0), 2)
                # Draw a rectangle around the ROI (pupil)
                cv2.rectangle(frame, (int(x_eye + x + x_offset), int(y_eye + y + y_offset)),
                              (int(x_eye + x + x_offset + roi_width), int(y_eye + y + y_offset + roi_height)),
                             (0, 0, 255), 2)
